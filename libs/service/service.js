...
module.exports = async (Service, config = {}, transports, osConfig = {}) => {
  ...
  assert(
    lodash.isFunction(Service),
    `Service is not a function: ${config.path} (${config.require})`
  );
  ...
  const clients = config.clients.reduce((result, client) => {
    ...
    assert(
      transport,
      "transport configuration not defined for: " + client.transport
    );
    assert(
      !lodash.has(result, path),
      "Conflict between service client names: " + path + " in " + config.name
    );
    lodash.set(
      result,
      path,
      Client({ ...osConfig, name: client.name }, transport, config.name)
    );
    return result;
  }, {});
